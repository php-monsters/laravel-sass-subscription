...
use LogicException;
use PhpMonsters\Sasscription\Events\FeatureConsumed;
use PhpMonsters\Sasscription\Events\FeatureTicketCreated;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('sasscription.models.feature_consumption'), 'subscriber');
    }

    public function featureTickets()
    {
        return $this->morphMany(config('sasscription.models.feature_ticket'), 'subscriber');
    }

    public function renewals()
    {
        return $this->hasManyThrough(
            config('sasscription.models.subscription_renewal'),
            config('sasscription.models.subscription'),
            'subscriber_id',
        );
    }
    ...
    {
        return $this->morphOne(config('sasscription.models.subscription'), 'subscriber')->ofMany('started_at', 'MAX');
    }

    public function lastSubscription()
    {
        return app(config('sasscription.models.subscription'))
            ->withExpired()
    ...
    {
        throw_unless(
            config('sasscription.feature_tickets'),
    ...
    {
        if (! config('sasscription.feature_tickets')) {
